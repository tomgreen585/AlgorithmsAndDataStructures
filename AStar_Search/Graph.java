...
public class Graph {
    ...
    private void createAndConnectEdges() {
        ...
        for (Line line : lines) {
            ...
            for (int i = 0; i < stops.size()-1; i++) { // stops on line
                
                Stop fromS = stops.get(i); // current stop
                Stop toS = null; // next null
                ...
                if (toS != null) { // create edge - toStop not empty
                edge = new Edge(fromS, toS, line.getType(), line, time, fromS.distanceTo(toS));
                }

                if (edge != null) { //add to edge set
                    edges.add(edge);               
                    fromS.addForwardEdge(edge); 
                    toS.addBackwardEdge(edge);  
                }
            }
        }
    }
     ...
     */
     ...
     public void computeNeighbours(){
        for (Stop stop : stops) {
            ...
            for (Edge edge : stop.getForwardEdges()) { // all forwardedges - adds stops
                stop.addNeighbour(edge.toStop());
            }
            
            for (Edge edge : stop.getBackwardEdges()) { // all backwardedges - adds stops
                stop.addNeighbour(edge.fromStop());
            }
        }
    }
        ...
        for (Stop fromS : stops) {
            ...
            for (Stop toS : stops) {
                ...
                if (fromS.distanceTo(toS) <= walkingDistance && fromS != toS) {
                    ...
                    Edge edge = new Edge(fromS, toS, Transport.WALKING, null, fromS.distanceTo(toS)/Transport.WALKING_SPEED_MPS, fromS.distanceTo(toS));
